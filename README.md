...
**Reminder for this release:** this release bumps the service-worker cache to v26 and adds team logos next to player names on board rows, compare cards, and roster tiles, dropping team text from the meta lines now that the logo carries it. Already-installed iPhones pick up the update on their SECOND online open after you deploy, not the first -- that's expected iOS service-worker behavior, not a bug.
...
A small tag that can appear next to an available player's name. It's a passive marker, never a suggestion on who to draft; no league setup needed:
...
Sources are weighted, not averaged equally -- Flock 33%, Sleeper 22%, Underdog 22%, ESPN 23%. Flock is the user's rankings site's expert consensus and Underdog comes from that same sheet (`scripts/board-source/adp.csv`); ESPN and Sleeper are live. That sheet also carries Yahoo, CBS, and FFPC columns, but none of the three enter the blend. The weighting lives in one constant, `ADP_WEIGHTS` in `js/state.js`, if you ever want to tilt it further toward one site. Flock and Underdog are rank-scale integers straight off the sheet, while ESPN and Sleeper are live decimal ADP -- deliberately mixed, near-identical scales.

`build-board.ps1` uses this same weighting to slot K/DST into the board by ADP (see Import your real rankings, above); a weight change here doesn't move their placement on its own -- that only updates at the next re-bake.
...
It shows in two places: a filled green badge on board rows, right after the purple ADP badge on the meta line, and its own "DK Proj" row on compare cards and the photo-tap peek card. The gate is strict, not partial: DK Proj only resolves to a number when DraftKings' lines -- plus any manual fills, see Manual lines, below -- cover every line that player's position requires: QB needs pass yards/TDs and rush yards/TDs; RB needs rush yards/TDs plus receptions/rec yards/rec TDs; WR and TE need receptions/rec yards/rec TDs. A player missing even one of those required lines skips the board badge -- not just a player with zero DK data at all -- and the compare/peek card shows an em-dash in its DK Proj row instead. Right now that gate leaves roughly 34 of the top 60 players showing a number; the rest fill in as manual lines land or DraftKings prices what's still missing.
...
**Manual lines:** DraftKings doesn't price every player, and the strict gate above means a partial line is as good as no line. When DK is missing a line, screenshot it from any sportsbook and send it in chat -- it gets transcribed into `scripts/dk-manual-lines.json`. The next `update-dk-lines.ps1` run merges those fills in, but only into whatever component DK itself didn't post that day: a fresh DK line always wins over a manual fill for the same player and stat. Every run also prints a rank-ordered gap worksheet of exactly which required lines are still missing, so you always know who to screenshot next.
